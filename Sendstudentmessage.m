...
@interface Sendstudentmessage ()<CDRTranslucentSideBarDelegate>{
    BOOL test,chk;
    NSString *Profile;
    ...
    NSMutableArray *arrclass,*arrgetcount;
    NSMutableArray *arrimg;
    NSMutableArray *arryList;
    NSMutableArray *arr;
    NSString *class;
    NSInteger takeid;
    NSMutableArray *arrparid;
    NSMutableArray *arrparname;
    NSMutableArray *arraddid;
...
@implementation Sendstudentmessage
...
- (void)viewDidLoad {
    ...
    tblStudentMessage.layer.borderWidth = 1;
    tblStudentMessage.layer.borderColor=[[UIColor colorWithRed:12.0/255.0 green:158.0/255.0 blue:239.0/255.0 alpha:1]CGColor];
    
    tblPopupStudentmessage.layer.borderWidth = 1;
    tblPopupStudentmessage.layer.borderColor=[[UIColor colorWithRed:12.0/255.0 green:158.0/255.0 blue:239.0/255.0 alpha:1]CGColor];
    
    tvStudentMessage.layer.borderWidth=1;
    tvStudentMessage.layer.borderColor=[[UIColor colorWithRed:12.0/255.0 green:158.0/255.0 blue:239.0/255.0 alpha:1]CGColor];
    tvStudentMessage.textColor=[UIColor lightGrayColor];
    tvStudentMessage.font=[UIFont fontWithName:@"RawengulkBold" size:22];
    tvStudentMessage.text = @"Enter Message";

    viewStudentMessage.layer.borderWidth = 1;
    viewStudentMessage.layer.borderColor=[[UIColor colorWithRed:12.0/255.0 green:158.0/255.0 blue:239.0/255.0 alpha:1]CGColor];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    {
        [tvStudentMessage resignFirstResponder];
...
- (BOOL) textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    else if([tvStudentMessage.text isEqualToString:@"Enter Message"])
    {
        tvStudentMessage.text=@"";
    }
    ...
    tvStudentMessage.textColor = [UIColor darkGrayColor];
    chk=NO;
    return YES;
}
...
{
    if(tvStudentMessage.text.length == 0)
    {
        tvStudentMessage.textColor = [UIColor lightGrayColor];
        tvStudentMessage.text =@"Enter Message";
        [tvStudentMessage resignFirstResponder];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if (tableView==tblStudentMessage)
    {
        return 1;
    }
    else if (tableView==tblPopupStudentmessage)
    {
        return 1;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==tblStudentMessage)
    {
        ...
        if (ce == nil)
        {
         ...
         ce.lblcount.text=[arrcount objectAtIndex:indexPath.row];
        ce.lblStudentName.text=[NSString stringWithFormat:@"%@",arryList[indexPath.row]];
    ...
    else if (tableView==tblPopupStudentmessage)
    {
        ...
        ce.lblStudentName.text=[NSString stringWithFormat:@"%@",arrparname[indexPath.row]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    {
        [tvStudentMessage resignFirstResponder];
        takeid=indexPath.row;
        [tblStudentMessage setHidden:YES];
        [tblPopupStudentmessage setHidden:YES];
        [tvStudentMessage setHidden:YES];
        [btnObjSend setHidden:YES];
        [imgPlan setHidden:YES];
        [lblObjMessage setHidden:YES];
        [viewStudentMessage setHidden:NO];
        CATransition *applicationLoadViewIn =[CATransition animation];
        [applicationLoadViewIn setDuration:0.5];
        [applicationLoadViewIn setType:kCATransitionReveal];
        [applicationLoadViewIn setTimingFunction:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn]];
        [[viewStudentMessage layer]addAnimation:applicationLoadViewIn forKey:kCATransitionReveal];
        [self getParent];
        //        Stuid=indexPath.row;
    ...
    else if(tableView==tblPopupStudentmessage)
    {
        ...
        [tblPopupStudentmessage reloadData];
    }
    else if(tableView==tblProfile)
    {
...
-(void)getClass
{
    ...
    [SVProgressHUD showProgress:0 status:@"Loading"];
    NSString *myRequestString1 = [NSString stringWithFormat:@"%@classdisplay.php?branch_id=%@&teacher_id=%@",[[Singleton sharedSingleton] getBaseURL],[prefs stringForKey:@"branch_id"],[prefs stringForKey:@"id"]];
    ...
    [operation1 setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[dictionary1 valueForKey:@"status"] isEqualToString:@"YES"] )
        {
            for(i=0;i<[[dictionary1 valueForKey:@"Post"]count];i++)
            {
                [arryList addObject:[NSString stringWithFormat:@"%@",[[[dictionary1 valueForKey:@"Post"]objectAtIndex:i]valueForKey:@"name"]]];
...
-(void)getParent
{
    arrparid=[[NSMutableArray alloc]init];
    arrparname=[[NSMutableArray alloc]init];
    arrimg=[[NSMutableArray alloc]init];
    
    [SVProgressHUD showProgress:0 status:@"Loading"];
    NSString *myRequestString1 = [NSString stringWithFormat:@"%@parentdisplay.php?class_id=%@",[[Singleton sharedSingleton] getBaseURL],[ClassId objectAtIndex:takeid]];
    NSString *str1 = [myRequestString1 stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"%@",str1);
    NSURL *url1 = [NSURL URLWithString:str1];
    NSURLRequest *request1 = [NSURLRequest requestWithURL:url1];
    ...
    [operation1 setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([[dictionary1 valueForKey:@"status"] isEqualToString:@"YES"] ) {
            for(i=0;i<[[dictionary1 valueForKey:@"Post"]count];i++)
            {
                [arrparid addObject:[NSString stringWithFormat:@"%@~%@",[[[dictionary1 valueForKey:@"Post"]objectAtIndex:i]valueForKey:@"parent_id"],[[[dictionary1 valueForKey:@"Post"]objectAtIndex:i]valueForKey:@"stud_id"]]];
                [arrparname addObject:[NSString stringWithFormat:@"%@",[[[dictionary1 valueForKey:@"Post"]objectAtIndex:i]valueForKey:@"name"]]];
            }
            
            if ([[arrgetcount objectAtIndex:takeid] isEqual:@"0"])
            {
                for (j=0; j<arrparid.count; j++)
                {
                    [arrimg addObject:@"0"];
                }
            }
            ...
            {
              arrimg = [[[arrgetcount objectAtIndex:takeid] componentsSeparatedByString: @"~"]mutableCopy];
        ...
        [tblPopupStudentmessage reloadData];
        [tblPopupStudentmessage setHidden:NO];
        [SVProgressHUD dismiss];
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"Error Retrieving Data" message:[error localizedDescription] confirmButtonTitle:NULL cancelButtonTitle:@"OK"];
...
- (IBAction)btnClickedSend:(id)sender {
    ...
    if (chk==YES)
    {
        ...
        [alertView handleCancel:^
         {
             [tvStudentMessage becomeFirstResponder];
    ...
    else if([tvStudentMessage.text isEqual:@"Enter Message"])
    {
        TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"SMBConnect" message:@"Enter Message" confirmButtonTitle:NULL cancelButtonTitle:@"OK"];
    ...
    else if([tvStudentMessage.text isEqual:@""])
    {
        TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"SMBConnect" message:@"Enter Message" confirmButtonTitle:NULL cancelButtonTitle:@"OK"];
    ...
    {
        TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"" message:@"Select Student" confirmButtonTitle:NULL cancelButtonTitle:@"OK"];
    ...
    else
    {
        ...
        NSMutableArray *arrsid=[[NSMutableArray alloc]init];
        
        for (int k = 0; k<arrsend.count; k++)
        {
            NSArray* dateArray = [[arrsend objectAtIndex:k]componentsSeparatedByString: @"~"];
            [arrpid addObject:[dateArray objectAtIndex: 0]];
            [arrsid addObject:[dateArray objectAtIndex: 1]];
            
            NSDictionary *jsonDictionary = [NSDictionary dictionaryWithObjectsAndKeys:arrpid[k], @"parent_id",arrsid[k], @"stud_id",nil];
        ...
        NSLog(@"%@",jsonString);
        NSString *appurl =[NSString stringWithFormat:@"%@parentsendpush.php?",[[Singleton sharedSingleton] getBaseURL]];
        ASIFormDataRequest *requestsASI = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:appurl]];
        
        [requestsASI addPostValue:@"1" forKey:@"branch_id"];
        [requestsASI addPostValue:@"3" forKey:@"teacher_id"];
        [requestsASI addPostValue:jsonString forKey:@"parent_id"];
        [requestsASI addPostValue:tvStudentMessage.text forKey:@"message"];
        ...
        {
            TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"" message:[result valueForKey:@"message"] confirmButtonTitle:NULL cancelButtonTitle:@"Ok"];
            ...
            [alertView show];
            [tvStudentMessage setText:@"Enter Message"];
            [self getClass];
            test=YES;
            j=0;
//            lblTotal.text=[NSString stringWithFormat:@"Total Student Selected                             %d",j];
        }
        else
        {
            TLAlertView *alertView = [TLAlertView showInView:self.view withTitle:@"" message:[result valueForKey:@"messege"] confirmButtonTitle:NULL cancelButtonTitle:@"Ok"];
...
- (IBAction)btnClickedPopupSend:(id)sender {
    ...
    [arrgetcount replaceObjectAtIndex:takeid withObject:[arrimg componentsJoinedByString:@"~"]];
     NSNumber *sum = [arrimg valueForKeyPath:@"@sum.floatValue"];
    [arrcount replaceObjectAtIndex:takeid withObject:[NSString stringWithFormat:@"%@",sum]];
    NSNumber *sum1=[arrcount valueForKeyPath:@"@sum.floatValue"];
    lblcount.text=[NSString stringWithFormat:@"%@",sum1];
    for (i=0; i<arrimg.count; i++)
    {
        if ([[arrimg objectAtIndex:i] isEqual:@"1"])
        {
            if ([arrsend containsObject:[arrparid objectAtIndex:i]])
            {
                
            }
            else
            {
                [arrsend addObject:[arrparid objectAtIndex:i]];
            }
        }
        else
        {
            if ([arrsend containsObject:[arrparid objectAtIndex:i]])
            {
                 [arrsend removeObject:[arrparid objectAtIndex:i]];
            }
            else
            {
    ...
    [tblStudentMessage reloadData];
    [tblStudentMessage setHidden:NO];
    [tvStudentMessage setHidden:NO];
    [btnObjSend setHidden:NO];
    [imgPlan setHidden:NO];
    [lblObjMessage setHidden:NO];
    [viewStudentMessage setHidden:YES];
